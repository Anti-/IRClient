IRClient is a [graphical] IRC client written in Vala (still not ready for [daily] usage)

Authors/Contributers
	* Anti (Anti- on GitHub) for lead development
	* max (mrtrop on GitHub) for GTK interface
	* sam/Stanley (sam0 on GitHub) for development
	* nemequ on Vala's IRC channel for assistance with errors
	
Currently in development as of Oct. 8, 2012:
	* joinChannel method in ClientBase class
	* Asynchronous functions
	* Graphical interface
	* Switch from multiple HashTables to one with class stuff

Current bugs:
	* Segmentation fault (core dumped) error sometimes happens, if not, nothing happens except lag
